...
namespace ViClass.Areas.Identity.Pages.Account
{
    ...
    public class RegisterModel : PageModel
    {
        ...
        public class InputModel : IValidatableObject
        {
            ...
            {
                if (!Regex.IsMatch(Password, @"^.*(?=.{6,})(?=.*[a-z])(?=.*[A-Z])(?=.*\d).*$"))
                    yield return new ValidationResult(
             "پسورد باید 6 کارکتر و شامل حداقل یک عدد, یک حرف کوچک, و یک حرف بزرگ باشد.", new []{nameof(Password)});
            }
        }
        ...
        public async Task<IActionResult> OnPostAsync(string returnUrl = null)
        {
            ...
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser
                    {UserName = Input.Email, Email = Input.Email, StudentNumber = Input.StudentNumber.ToString()};
